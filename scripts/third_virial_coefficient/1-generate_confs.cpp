...
const double max_dist = 30;  // Angstrom
double emin = 0.0;
...
int main(int argc, char** argv) {
    ...
    std::ofstream ofss("index-distance_" + std::to_string(r12str) + "_" + std::to_string(r12end) + ".txt");

    for (double R1 = r12str; R1 < r12end + dR / 2.0; R1 += dR) {
        count = confsxr12 * (int((R1 + dR / 2.0) / dR) - 1) + 1;
        for (double R2 = dR; R2 < max_dist + dR / 2.0; R2 += dR) {
            std::cout << "Working on d12 = " << R1 << " and d13 = " << R2 << std::endl;
            ofss << std::setw(10) << count << std::setw(15) << R1 << std::setw(15) << R2 << std::endl;
