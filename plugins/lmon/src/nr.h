...
void amebsa(Mat_IO_DP &p, Vec_IO_DP &y, Vec_O_DP &pb, DP &yb, const DP ftol, DP funk(Vec_I_DP &), int &iter,
            const DP temptr);
...
DP amotsa(Mat_IO_DP &p, Vec_O_DP &y, Vec_IO_DP &psum, Vec_O_DP &pb, DP &yb, DP funk(Vec_I_DP &), const int ihi, DP &yhi,
          const DP fac);
...
void arcsum(Vec_I_ULNG &iin, Vec_O_ULNG &iout, unsigned long ja, const int nwk, const unsigned long nrad,
...
void bcuint(Vec_I_DP &y, Vec_I_DP &y1, Vec_I_DP &y2, Vec_I_DP &y12, const DP x1l, const DP x1u, const DP x2l,
            const DP x2u, const DP x1, const DP x2, DP &ansy, DP &ansy1, DP &ansy2);
...
void bsstep(Vec_IO_DP &y, Vec_IO_DP &dydx, DP &xx, const DP htry, const DP eps, Vec_I_DP &yscal, DP &hdid, DP &hnext,
...
void difeq(const int k, const int k1, const int k2, const int jsf, const int is1, const int isf, Vec_I_INT &indexv,
           Mat_O_DP &s, Mat_I_DP &y);
...
void fasper(Vec_I_DP &x, Vec_I_DP &y, const DP ofac, const DP hifac, Vec_O_DP &wk1, Vec_O_DP &wk2, int &nout, int &jmax,
            DP &prob);
...
void fitexy(Vec_I_DP &x, Vec_I_DP &y, Vec_I_DP &sigx, Vec_I_DP &sigy, DP &a, DP &b, DP &siga, DP &sigb, DP &chi2,
            DP &q);
...
DP fredin(const DP x, const DP a, const DP b, Vec_I_DP &t, Vec_I_DP &f, Vec_I_DP &w, DP g(const DP),
...
void hypser(const complex<DP> &a, const complex<DP> &b, const complex<DP> &c, const complex<DP> &z, complex<DP> &series,
            complex<DP> &deriv);
...
void lfit(Vec_I_DP &x, Vec_I_DP &y, Vec_I_DP &sig, Vec_IO_DP &a, Vec_I_BOOL &ia, Mat_O_DP &covar, DP &chisq,
...
void lnsrch(Vec_I_DP &xold, const DP fold, Vec_I_DP &g, Vec_IO_DP &p, Vec_O_DP &x, DP &f, const DP stpmax, bool &check,
            DP func(Vec_I_DP &));
...
void machar(int &ibeta, int &it, int &irnd, int &ngrd, int &machep, int &negep, int &iexp, int &minexp, int &maxexp,
            DP &eps, DP &epsneg, DP &xmin, DP &xmax);
...
void mmid(Vec_I_DP &y, Vec_I_DP &dydx, const DP xs, const DP htot, const int nstep, Vec_O_DP &yout,
          void derivs(const DP, Vec_I_DP &, Vec_O_DP &));
...
void mrqcof(Vec_I_DP &x, Vec_I_DP &y, Vec_I_DP &sig, Vec_I_DP &a, Vec_I_BOOL &ia, Mat_O_DP &alpha, Vec_O_DP &beta,
            DP &chisq, void funcs(const DP, Vec_I_DP &, DP &, Vec_O_DP &));
void mrqmin(Vec_I_DP &x, Vec_I_DP &y, Vec_I_DP &sig, Vec_IO_DP &a, Vec_I_BOOL &ia, Mat_O_DP &covar, Mat_O_DP &alpha,
            DP &chisq, void funcs(const DP, Vec_I_DP &, DP &, Vec_O_DP &), DP &alamda);
void newt(Vec_IO_DP &x, bool &check, void vecfunc(Vec_I_DP &, Vec_O_DP &));
void odeint(Vec_IO_DP &ystart, const DP x1, const DP x2, const DP eps, const DP h1, const DP hmin, int &nok, int &nbad,
            void derivs(const DP, Vec_I_DP &, Vec_O_DP &),
            void rkqs(Vec_IO_DP &, Vec_IO_DP &, DP &, const DP, const DP, Vec_I_DP &, DP &, DP &,
...
void period(Vec_I_DP &x, Vec_I_DP &y, const DP ofac, const DP hifac, Vec_O_DP &px, Vec_O_DP &py, int &nout, int &jmax,
            DP &prob);
void piksr2(Vec_IO_DP &arr, Vec_IO_DP &brr);
void piksrt(Vec_IO_DP &arr);
void pinvs(const int ie1, const int ie2, const int je1, const int jsf, const int jc1, const int k, Mat3D_O_DP &c,
           Mat_IO_DP &s);
...
void red(const int iz1, const int iz2, const int jz1, const int jz2, const int jm1, const int jm2, const int jmf,
         const int ic1, const int jc1, const int jcf, const int kc, Mat3D_I_DP &c, Mat_IO_DP &s);
...
void rk4(Vec_I_DP &y, Vec_I_DP &dydx, const DP x, const DP h, Vec_O_DP &yout,
         void derivs(const DP, Vec_I_DP &, Vec_O_DP &));
void rkck(Vec_I_DP &y, Vec_I_DP &dydx, const DP x, const DP h, Vec_O_DP &yout, Vec_O_DP &yerr,
          void derivs(const DP, Vec_I_DP &, Vec_O_DP &));
void rkdumb(Vec_I_DP &vstart, const DP x1, const DP x2, void derivs(const DP, Vec_I_DP &, Vec_O_DP &));
void rkqs(Vec_IO_DP &y, Vec_IO_DP &dydx, DP &x, const DP htry, const DP eps, Vec_I_DP &yscal, DP &hdid, DP &hnext,
          void derivs(const DP, Vec_I_DP &, Vec_O_DP &));
...
void simpr(Vec_I_DP &y, Vec_I_DP &dydx, Vec_I_DP &dfdx, Mat_I_DP &dfdy, const DP xs, const DP htot, const int nstep,
...
void solvde(const int itmax, const DP conv, const DP slowc, Vec_I_DP &scalv, Vec_I_INT &indexv, const int nb,
            Mat_IO_DP &y);
...
void stifbs(Vec_IO_DP &y, Vec_IO_DP &dydx, DP &xx, const DP htry, const DP eps, Vec_I_DP &yscal, DP &hdid, DP &hnext,
            void derivs(const DP, Vec_I_DP &, Vec_O_DP &));
void stiff(Vec_IO_DP &y, Vec_IO_DP &dydx, DP &x, const DP htry, const DP eps, Vec_I_DP &yscal, DP &hdid, DP &hnext,
           void derivs(const DP, Vec_I_DP &, Vec_O_DP &));
void stoerm(Vec_I_DP &y, Vec_I_DP &d2y, const DP xs, const DP htot, const int nstep, Vec_O_DP &yout,
            void derivs(const DP, Vec_I_DP &, Vec_O_DP &));
void svbksb(Mat_I_DP &u, Vec_I_DP &w, Mat_I_DP &v, Vec_I_DP &b, Vec_O_DP &x);
void svdcmp(Mat_IO_DP &a, Vec_O_DP &w, Mat_O_DP &v);
void svdfit(Vec_I_DP &x, Vec_I_DP &y, Vec_I_DP &sig, Vec_O_DP &a, Mat_O_DP &u, Mat_O_DP &v, Vec_O_DP &w, DP &chisq,
            void funcs(const DP, Vec_O_DP &));
...
void vegas(Vec_I_DP &regn, DP fxn(Vec_I_DP &, const DP), const int init, const int ncall, const int itmx,
           const int nprn, DP &tgral, DP &sd, DP &chi2a);
void voltra(const DP t0, const DP h, Vec_O_DP &t, Mat_O_DP &f, DP g(const int, const DP),
