...
void print_exchange(std::ostream&, size_t,
    const double& Told, const double& Tpartner, const double& Tnew);
...
int main(int argc, char** argv)
{
    ...
    if (comm_replica_rank == 0)
        pimd.m_virials = new double[pimd.natom()];
    ...
    if (comm_replica_rank == 0) {
        ...
        if (!os) {
            std::cerr << " ** Error ** : could not open '"
                      << output << "' for writing " << std::endl;

            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }

        if (ntraj > 0)
            os_traj.open(trajectory.c_str());

        if (!os_traj) {
            std::cerr << " ** Error ** : could not open '"
                      << trajectory << "' for writing " << std::endl;

            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }

        if (nvirials > 0)
            os_virials.open(virials.c_str());

        if (!os_virials) {
            std::cerr << " ** Error ** : could not open '"
                      << virials << "' for writing " << std::endl;
        ...
        os << "# replica exchange PIMD ["
           //<< PACKAGE_VERSION << "]"
           << "\n# === this is replica " << (replica_id + 1)
                      << " of " << nreplicas << " running on "
    ...
    for (size_t step = initial_step; step < nstep; ++step) {
        ...
        if (comm_replica_rank == 0) {
            ...
            const bool need_to_print_energies =
                (nprint > 0 && step > 0 && (step + 1)%nprint == 0);

            const bool need_to_save_virials =
                (nvirials > 0 && step > 0 && (step + 1)%nvirials == 0);

            if (need_to_print_energies || need_to_save_virials)
                pimd.compute_virials();

            if (nreposition > 0 && step%nreposition == 0)
                pimd.set_COM_at_origin();

            if (need_to_print_energies)
                pimd.print_energies(os, step + 1);

            if (ntraj > 0 && step > 0 && (step + 1)%ntraj == 0)
                pimd.save_positions(os_traj, step + 1);

            if (need_to_save_virials)
                pimd.save_virials(os_virials, step + 1);

            if (step > 0 && step%nexchange == 0
                && comm_masters_rank != MPI_UNDEFINED)
                pimd.try_exchange(os, step);
...
void setup_comms(MPI_Comm comm)
{
    ...
    if (world_size%nreplicas != 0 || world_size < int(nreplicas)) {
        if (world_rank == 0)
            std::cerr
              << " ** Error ** : incommensurate number of threads/replicas"
              << std::endl;
...
void print_exchange(std::ostream& os, size_t step,
    const double& Tbefore, const double& Tpartner, const double& Tafter)
{
    using namespace std;
...
void my_pimd::print_energies(std::ostream& os, size_t step)
{
    ...
    os << setprecision(4)
       << setw(8) << step << ' '
       << fixed << showpoint << setw(10) << (step*dt) << ' '
       //<< fixed << showpoint << setw(10) << (m_time0 + step*dt) << ' '
       << setprecision(4) << scientific
       << setw(11) << (m_Epot_sum/engunit) << ' '
       << setw(11) << (m_Espring/engunit) << ' '
       << setw(11) << Ekin_CV << ' '
       << setw(11) << (m_Ekin_fict/engunit) << ' '
       << setw(11) << (m_Econ_sum/engunit) << ' '
       << setw(11) << (invariant()/engunit)
       << '\n';
...
void my_pimd::set_COM_at_origin()
{
    ...
    for (size_t n = 0; n < natom(); ++n) {
        ...
        for (size_t k = 0; k < 3; ++k)
            com[k] += mass(n)*m_pos_nmode[3*n + k];
    }
...
void my_pimd::try_exchange(std::ostream& os, size_t step)
{
    ...
    for (int n = 0; n < np; n += 2)
        if (permutation[n] < comm_masters_size &&
            permutation[n + 1] < comm_masters_size) {
    ...
    MPI_Sendrecv(my, 3, MPI_DOUBLE, partner_rank, 8,
                 his, 3, MPI_DOUBLE, partner_rank, 8,
                 comm_masters, MPI_STATUS_IGNORE);
    ...
    if (comm_masters_rank > partner_rank) {
        ...
        const double delta =
            (his[0] - my[0])*(my_beta - his_beta)
          + (his[1]*his_beta*his_beta - my[1]*my_beta*my_beta)*(my_kT - his_kT);

        if (delta < 0.0
            || (delta >= 0.0 && std::exp(-delta) > random_number))
            do_exchange = 1;

        MPI_Send(&do_exchange, 1, MPI_INT, partner_rank,
                 9, comm_masters);

    } else
        MPI_Recv(&do_exchange, 1, MPI_INT, partner_rank,
                 9, comm_masters, MPI_STATUS_IGNORE);

    // complete the exchange if accepted

    if (do_exchange)
        change_temperature(his[2]);
...
void my_pimd::save_positions(std::ostream& os, size_t step)
{
    ...
    os << kit::io::sec("time", dt*step)
       << kit::io::sec("temperature", m_T)
       << kit::io::sec("mass", natom(), m_fict_mass)
       << kit::io::sec("positions", 3*natom()*nbead(), m_pos_cart)
       << kit::io::endframe();
}
...
void my_pimd::save_virials(std::ostream& os, size_t step)
{
    ...
    os << kit::io::sec("time", dt*step)
       << kit::io::sec("temperature", m_T)
       << kit::io::sec("mass", natom(), m_fict_mass)
       << kit::io::sec("virials", natom(), m_virials)
       << kit::io::endframe();
}
