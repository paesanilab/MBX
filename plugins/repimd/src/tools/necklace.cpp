...
{
    if (natoms == m_natom && nbeads == m_nbead)
        return;
    ...
    if (natoms > 0 || nbeads > 0) {
...

        {
            ...
            m_plan_cart2nmode = fftw_plan_many_r2r
                (1, &n, howmany,
                 ...
                 &c2n_kind, FFTW_MEASURE | FFTW_PRESERVE_INPUT);
        ...
        for (size_t n = 1; n < nbeads/2; ++n)
            m_lambda[nbeads - n] = m_lambda[n] =
                4*nbeads*(1.0 - std::cos(2*n*(M_PI/nbeads)));
        ...
        for (size_t n = 1; n < nbeads/2; ++n)
            m_lambda[2*n - 1] = m_lambda[2*n] =
                2*nbeads*(1.0 - std::cos(2*n*(M_PI/nbeads)));
...
void necklace::pos_c2n()
{
    ...
    for (size_t n = 0; n < 3*m_nbead*m_natom; ++n)
    ...
    for (size_t i = 0; i < m_nbead; ++i) {
        ...
        for (size_t j = 0; j < m_nbead; ++j) {
            ...
            for (size_t k = 0; k < natom3; ++k)
                m_pos_nmode[i0 + k] += factor*m_pos_cart[j0 + k];
        }
    }
...
void necklace::pos_n2c()
{
    ...
    for (size_t i = 0; i < m_nbead; ++i) {
        ...
        for (size_t j = 0; j < m_nbead; ++j) {
            ...
            for (size_t k = 0; k < natom3; ++k)
                m_pos_cart[i0 + k] += factor*m_pos_nmode[j0 + k];
        }
    }
#endif /* ENABLE_FFTW3 */
}
...
void necklace::frc_c2n()
{
    ...
    for (size_t i = 0; i < m_nbead; ++i) {
        ...
        for (size_t j = 0; j < m_nbead; ++j) {
            ...
            const double factor = M_N2C(j, i)/m_nbead;
            for (size_t k = 0; k < natom3; ++k)
                m_frc_nmode[i0 + k] += factor*m_frc_cart[j0 + k];
        }
    }
