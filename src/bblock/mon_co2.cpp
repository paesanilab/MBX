...
CO2::CO2(double * coords, std::vector<std::string> names) {
  ...
  char rel_path[80] = "/src/potential/1b/params/mon_co2.dat";
...
double CO2::Calc1BEnergy(double * syscoords) {
...
double CO2::Calc1BEnergy(double * syscoords, double * grad) {
  // TODO: Update not necessary. We can check it later
  std::copy(syscoords + 3 * first_index, 
            syscoords + 3 * (first_index + n_sites),  xyz.get());
