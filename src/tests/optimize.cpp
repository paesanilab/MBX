...
void run_opt(std::vector<double> xyz, std::vector<double> &xyz_opt,
             double &grms, double &grad2, double tol, size_t maxit, size_t numats,
             double &e0, double &e1) {
...
int main(int argc, char** argv)
{
  ...
  if (argc < 2) {
    std::cout << "Usage: " << argv[0] << " <config.nrg> [<mbx.json>] [<max iterations>] [<tolerance>] [<configs.xyz>]\n";
    return 0;
  }
