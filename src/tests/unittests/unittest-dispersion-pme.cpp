...
TEST_CASE("Test the dispersion class.") {
    ...
    std::vector<double> box = {30.0,0.0,0.0,0.0,30.0,0.0,0.0,0.0,30.0};
    ...
    SECTION("Check continuity of the energy across switching region and cutoff") {
        ...
        for(double coord = start + delta; coord < CUTOFF+0.2; coord += delta) {
            ...
            std::cout <<  std::setw(16) << std::setprecision(10) << coord << ",  "
                      <<  std::setw(16) << std::setprecision(10) << energy << ",  "
                      <<  std::setw(16) << std::setprecision(10) << dE << std::endl;
            REQUIRE(dE == Approx(0).margin(2e-5));
            old_energy = energy;
        }
    ...
    SECTION("Check energies and gradients are invariant to the Ewald alpha value"){
        ...
        ewald_disp.Initialize(C6_long_range, real_coords, monomer_names, n_atoms_vector, internal_mon_type_count, true, box);
