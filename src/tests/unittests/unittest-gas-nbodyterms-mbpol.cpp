...
void GetDimers(const std::vector<double> xyz, const size_t nmon, std::vector<double> &xyz1, std::vector<double> &xyz2, size_t &ndimers) {
    ndimers = 0;
    xyz1.clear();
    xyz2.clear();
...
TEST_CASE("Test the n-body terms for mbpol (gas phase).") {
    ...
    SECTION("One body (polynomials)") {
        ...
        SECTION("Energy without gradients.") {
             REQUIRE(energy_1b_nograd == Approx(3.2265445802e-01).margin(TOL));
        }

        SECTION("Energy with gradients") {
             REQUIRE(energy_1b_grad == Approx(3.2265445802e-01).margin(TOL));
        }
    ...
    SECTION("Two body (polynomials)") {
        ...
        SECTION("Energy without gradients.") {
             REQUIRE(energy_2b_nograd == Approx(1.7011845631e+01).margin(TOL));
        }

        SECTION("Energy with gradients") {
             REQUIRE(energy_2b_grad == Approx(1.7011845631e+01).margin(TOL));
        }
    ...
    SECTION("Two body (dispersion)") {
        ...
        double energy_2b_nograd = disp::GetDispersion(monomer_name,monomer_name,ndimers,false,xyz1,xyz2,grad1,grad2, 100.0,false);
        double energy_2b_grad = disp::GetDispersion(monomer_name,monomer_name,ndimers,true,xyz1,xyz2,grad1,grad2, 100.0,false);
        ...
        SECTION("Energy without gradients.") {
             REQUIRE(energy_2b_nograd == Approx(-8.1876933060e+00).margin(TOL));
        }

        SECTION("Energy with gradients") {
             REQUIRE(energy_2b_grad == Approx(-8.1876933060e+00).margin(TOL));
        }
        ...
        SECTION("Numerical gradients vs analitical gradients") {
            ...
            for (size_t molecule = 0; molecule < nmon; ++molecule) {
                for (size_t degreeOfFreedom = atomOffset; degreeOfFreedom < atomOffset + 9; ++degreeOfFreedom) {
                    real_xyz[degreeOfFreedom] += stepSize;
                    GetDimers(real_xyz,nmon,xyz1,xyz2,ndimers);
                    double plusEnergy = disp::GetDispersion(monomer_name,monomer_name,ndimers,false,xyz1,xyz2,grad1,grad2, 100.0,false);
                    real_xyz[degreeOfFreedom] -= 2 * stepSize;
                    GetDimers(real_xyz,nmon,xyz1,xyz2,ndimers);
                    double minusEnergy = disp::GetDispersion(monomer_name,monomer_name,ndimers,false,xyz1,xyz2,grad1,grad2, 100.0,false);
                    real_xyz[degreeOfFreedom] += stepSize;
                    double finiteDifferenceForce = (plusEnergy - minusEnergy) / (2 * stepSize);
                    double error = grad[degreeOfFreedom] - finiteDifferenceForce;
                    REQUIRE(grad[degreeOfFreedom] == Approx(finiteDifferenceForce).margin(TOL));
    ...
    SECTION("Three body (polynomials)") {
        ...
        double energy_3b_grad = e3b::get_3b_energy(monomer_name,monomer_name,monomer_name,1,xyz1,xyz2,xyz3,grad1,grad2,grad3);
        ...
        SECTION("Energy without gradients.") {
             REQUIRE(energy_3b_nograd == Approx(2.2777705108e-01).margin(TOL));
        }

        SECTION("Energy with gradients") {
             REQUIRE(energy_3b_grad == Approx(2.2777705108e-01).margin(TOL));
        }
        ...
        SECTION("Numerical gradients vs analitical gradients") {
            ...
            for (size_t molecule = 0; molecule < nmon; ++molecule) {
                for (size_t degreeOfFreedom = atomOffset; degreeOfFreedom < atomOffset + 9; ++degreeOfFreedom) {
                    ...
                    std::copy(real_xyz.begin() + 9, real_xyz.begin() + 18, xyz2.begin());
                    std::copy(real_xyz.begin() + 18, real_xyz.begin() + 27, xyz3.begin());
                    double plusEnergy = e3b::get_3b_energy(monomer_name,monomer_name,monomer_name,1,xyz1,xyz2,xyz3);
                    real_xyz[degreeOfFreedom] -= 2 * stepSize;
                    std::copy(real_xyz.begin(), real_xyz.begin() + 9, xyz1.begin());
                    std::copy(real_xyz.begin() + 9, real_xyz.begin() + 18, xyz2.begin());
                    std::copy(real_xyz.begin() + 18, real_xyz.begin() + 27, xyz3.begin());
                    double minusEnergy = e3b::get_3b_energy(monomer_name,monomer_name,monomer_name,1,xyz1,xyz2,xyz3);
                    real_xyz[degreeOfFreedom] += stepSize;
                    double finiteDifferenceForce = (plusEnergy - minusEnergy) / (2 * stepSize);
                    double error = grad[degreeOfFreedom] - finiteDifferenceForce;
                    REQUIRE(grad[degreeOfFreedom] == Approx(finiteDifferenceForce).margin(TOL));
