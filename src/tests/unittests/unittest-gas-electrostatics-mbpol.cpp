...
TEST_CASE("test the electrostatics class for mbpol (gas phase).") {
    ...
    SECTION("Total electrostatic energy and forces and individual terms") {
        ...
        REQUIRE(perm_elec == Approx(-1.8567131645e+01).margin(TOL));
        REQUIRE(ind_elec == Approx(-6.4991884968e+00).margin(TOL));

        for (size_t i = 0; i < grad.size(); i++)
            REQUIRE(forces[i] == Approx(grad[i]).margin(TOL));
