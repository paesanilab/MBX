...
#include "setup_h4_dummy_dihedral.h"
...
TEST_CASE("Test dummy h4 dihedral") {
    // Create the system
    SETUP_H4_DUMMY_DIHEDRAL

    SECTION("Dihedral-cos") {
        std::string functional_form = "cos";
        Dihedral obj(connectivity, dihedral_type, indexes, functional_form);
        obj.SetParameters(cos_linear_parameters, cos_nonlinear_parameters);
        SECTION("Cosine Energy") { REQUIRE(obj.GetEnergy(phi) == Approx(ff_dihedral_cos_energy).margin(TOL)); }
        ...
        SECTION("Cosine Gradient") {
            REQUIRE(obj.GetTopologyGradient(phi) == Approx(ff_dihedral_cos_grad).margin(TOL));
        }
    }
    ...
    SECTION("Dihedral-harmonic") {
        ...
        SECTION("Harmonic Energy") { REQUIRE(obj.GetEnergy(phi) == Approx(ff_dihedral_harm_energy).margin(TOL)); }
        ...
        SECTION("Harmonic Gradient") {
            REQUIRE(obj.GetTopologyGradient(phi) == Approx(ff_dihedral_harm_grad).margin(TOL));
        }
    }
    ...
    SECTION("Dihedral-hcos") {
        ...
        SECTION("Harmonic Cosine Energy") {
            REQUIRE(obj.GetEnergy(phi) == Approx(ff_dihedral_hcos_energy).margin(TOL));
        }
        ...
        SECTION("Harmonic Cosine Gradient") {
            REQUIRE(obj.GetTopologyGradient(phi) == Approx(ff_dihedral_hcos_grad).margin(TOL));
        }
    }
    ...
    SECTION("Dihedral-cos3") {
        ...
        SECTION("Cosine 3 Energy") { REQUIRE(obj.GetEnergy(phi) == Approx(ff_dihedral_cos3_energy).margin(TOL)); }
        ...
        SECTION("Cosine 3 Gradient") {
            REQUIRE(obj.GetTopologyGradient(phi) == Approx(ff_dihedral_cos3_grad).margin(TOL));
        }
    }
    ...
    SECTION("Dihedral-none") {
        ...
        SECTION("None Energy") { REQUIRE(obj.GetEnergy(phi) == Approx(ff_dihedral_none_energy).margin(TOL)); }
        ...
        SECTION("None Gradient") { REQUIRE(obj.GetTopologyGradient(phi) == Approx(ff_dihedral_none_grad).margin(TOL)); }
    }
    ...
    SECTION("Dihedral-include-exclude") {
        ...
        // SECTION("Get Include Exclude") { REQUIRE(obj.GetIncludeExclude() == include_exclude); };
    }
}
