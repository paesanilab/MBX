...
TEST_CASE("Test the system tools functions (no PBC).") {
    ...
    // Run OrderMonomers
    try {
        mon_type_count = systools::OrderMonomers(mon_names, sites_out, nat_out, original2current, orginal_order, orginal_order_realSites);
    } catch (CUException &e) {
        std::cerr << e.what();
    }
    ...
    SECTION("SetupMonomers") {
        ...
        SECTION("Sites Vector") {
            REQUIRE(VectorsAreEqual(sites_out,n_sites_vector));
        }

        SECTION("Number of Atoms Vector") {
            REQUIRE(VectorsAreEqual(nat_out,n_atoms_vector));
        }

        SECTION("First Index Vector") {
            REQUIRE(VectorsAreEqual(first_index_out,first_index_realSites));
        }
    ...
    SECTION("OrderMonomers") { 
        ...
        SECTION("Monomer type count") {
            REQUIRE(VectorsAreEqual(internal_mon_type_count,mon_type_count));
        }

        SECTION("Internal Order monomer names") {
            REQUIRE(VectorsAreEqual(mon_names,internal_monomer_names));
        }
        ...
        SECTION("Original Order Vector") {
            REQUIRE(VectorsAreEqual(orginal_order,internal_orginal_order));
        }
        ...
        SECTION("Monomer vector is empty") {
            ...
            try {
               mon_type_count2 = systools::OrderMonomers(monomers_empty, sites_out, nat_out, original2current2, orginal_order2, orginal_order_realSites2); 
            } catch (CUException &e) {
        ...
        SECTION("Monomer vector size not matching atom or sites vector") {
            ...
            try {
               mon_type_count2 = systools::OrderMonomers(monomer_names, sites2, nat_out, original2current2, orginal_order2, orginal_order_realSites2);
            } catch (CUException &e) {
            ...
            try {
               mon_type_count2 = systools::OrderMonomers(monomer_names, sites_out, nats2, original2current2, orginal_order2, orginal_order_realSites2);
            } catch (CUException &e) {
