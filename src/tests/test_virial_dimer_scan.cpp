...
int main(int argc, char** argv)
{
    ...
    for (size_t k = 0; k < 15; k++) {
        ...
        std::vector<double> ref(xyz.begin() + 15, xyz.begin() + 18);
        for (size_t j = 0; j < 5; j++) {
            for (size_t i = 0; i < 3; i++) { 
                xyz[15 + 3*j + i] = xyz[15 + 3*j + i] - ref[i] + xyz[i];
            }
            xyz[15 + 3*j] += d;
        }
        ...
        for (size_t i = 0; i < 3; i++) {
            std::cout << std::scientific << std::setprecision(6) << "virial 1 system = " << virial[3*i + i] << "  ; virial 1 forces = " << virial_forces[i] << "  ; diff = " << virial[3*i + i] - virial_forces[i] << std::endl;
        }
