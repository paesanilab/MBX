...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    std::cout << "Usage: " << argv[0] 
              << "<input.nrg> [<mbx.json> <configs.xyz>]" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::copy(xyz_nrg_coords.begin() + 3*num_at[0], xyz_nrg_coords.begin() + 3*(num_at[0] + num_at[1]), xyz.begin());
  ...
  if (!use_xyz) {
    double en3b = my_sys_t[0].Energy(false) 
                - my_sys_d1.Energy(false)
                - my_sys_d2.Energy(false)
                - my_sys_d3.Energy(false)
                + my_sys_t[0].OneBodyEnergy(false);
  ...
  for (size_t i = 0; i < ats.size(); i++) {
    ...
    threebody[i] = my_sys_t[0].Energy(false)
                 - my_sys_d1.Energy(false)
                 - my_sys_d2.Energy(false)
                 - my_sys_d3.Energy(false)
                 + my_sys_t[0].OneBodyEnergy(false);;
