...
void ElectricFieldHolder::CalcPermanentElecField_Optimized(
    ...
    double r_sq_raw[mon2_index_end - mon2_index_start];
    ...
    for (size_t m = mon2_index_start; m < mon2_index_end; m++) {
        ...
        r_sq_raw[m - mon2_index_start] = v3[m - mon2_index_start];
    ...
    #pragma omp simd simdlen(8) reduction(+ : v7, v8, v9, v10)
    for (size_t m = mon2_index_start; m < mon2_index_end; m++) {
        const size_t m_shifted = m - mon2_index_start;
        const bool use_ewald = use_pbc;
        const double ewaldterm = use_ewald ? 2 * exp_alpha2r2[m_shifted] * ewald_alpha / PIQSRT : 0;

        // Screening functions
        double s1r = v4[m_shifted] - exp1[m_shifted] * v3[m_shifted];
        double s0r = (s1r + aCC1_4 * Ai * g34 * v6[m_shifted]);
        s1r3[m_shifted] = (s1r + ewaldterm) * v3[m_shifted] * v3[m_shifted];
        ...
        if (use_softcore) {
            ...
            if (r_sq < cutoff * cutoff) {
                ...
                const double erf_term = use_ewald ? erf(ewald_alpha * r_raw) : 0.0;
                const double one_minus_lambda = 1.0 - elec_lambda;
                const double lambda_pow = std::pow(elec_lambda, kSoftcoreExponent);
                const double soft_denom = kSoftcoreAlphaC * one_minus_lambda * one_minus_lambda + r_sq;
                const double inv_r_eff = lambda_pow / std::sqrt(soft_denom);
                const double damped_potential_terms = elec_scale_factor - erf_term - exp1[m_shifted];
                s0r = damped_potential_terms * inv_r_eff + aCC1_4 * Ai * g34 * v6[m_shifted];

                if (r_raw > 0.0) {
                    const double term_A = (lambda_pow * r_raw) / std::pow(soft_denom, 1.5);
                    const double term_B = damped_potential_terms / r_raw + ewaldterm;
                    s1r3[m_shifted] = term_A * term_B;
                } else {
                    s1r3[m_shifted] = 0.0;
                }
            }
        ...
        // Assuming phi will be at1mon1_index at1m2 at1m3 .. for same type of mons
        v7 += scale[m_shifted] * s0r * chg_j;
        phi2[site_jnmon2 + m] += scale[m_shifted] * s0r * chg_i;

        // Field will be as xyz xxxxyyyyzzzzat1 xxxxxyyyyzzzz at2...
        const double s1r3ci = scale[m_shifted] * s1r3[m_shifted] * chg_i;
        const double s1r3cj = scale[m_shifted] * s1r3[m_shifted] * chg_j;

        // Compute the three components of the permanent electric field
        // Storing contributions to mon1 in vectors to make the loop vectorizable

        v8 += s1r3cj * rijx[m_shifted];
        v9 += s1r3cj * rijy[m_shifted];
        v10 += s1r3cj * rijz[m_shifted];

        Efq2[site_jnmon23 + m] -= s1r3ci * rijx[m_shifted];
        Efq2[site_jnmon23 + nmon2 + m] -= s1r3ci * rijy[m_shifted];
        Efq2[site_jnmon23 + nmon22 + m] -= s1r3ci * rijz[m_shifted];
