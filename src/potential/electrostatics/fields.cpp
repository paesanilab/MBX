    ...
    for (size_t m = mon2_index_start; m < mon2_index_end; m++) {
        ...
        if (accum2) {
            ...
            if (use_pbc) {
                ...
                // Convert back to Cartesian coordinates
                rijx = box[0] * minfracrijx + box[3] * minfracrijy + box[6] * minfracrijz;
                rijy = box[1] * minfracrijx + box[4] * minfracrijy + box[7] * minfracrijz;
                rijz = box[2] * minfracrijx + box[5] * minfracrijy + box[8] * minfracrijz;
            } else {
                rijx = rawrijx;
                rijy = rawrijy;
                rijz = rawrijz;
...
// TODO look at the exponential function intel vec
                ...
                const double ts2x = scale * s2r5_3 * rijx;
                const double ts2y = scale * s2r5_3 * rijy;
                const double ts2z = scale * s2r5_3 * rijz;
                ...
                // Component z
                double tmpz = (ts2z * rijx) * mu1[site_inmon13 + mon1_index] +
