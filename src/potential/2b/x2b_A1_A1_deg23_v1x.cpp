...
x2b_A1_A1_v1x::x2b_A1_A1_v1x(std::string mon1, std::string mon2) {
    ...
    if (mon1 == "he" && mon2 == "he") {
        coefficients = std::vector<double>{1.740284808237542e-02,    // 0
                                           -6.500470848631896e-01,   // 1
                                           1.108964734872929e+01,    // 2
                                           1.060893156799291e+00,    // 3
                                           -1.720336052224201e+00,   // 4
                                           -7.964002590111028e-01,   // 5
                                           3.626312160634779e-01,    // 6
                                           6.943622224620387e-01,    // 7
                                           2.837583131890949e-01,    // 8
                                           -3.287341231862393e-01,   // 9
                                           -4.905233703907272e-01,   // 10
                                           4.014044918678196e-02,    // 11
                                           5.263451503780592e-01,    // 12
                                           -3.425642727268442e-02,   // 13
                                           -5.839964847928830e-01,   // 14
                                           6.010641655008873e-01,    // 15
                                           -3.153645111123357e-01,   // 16
                                           1.040082750603098e-01,    // 17
                                           -2.284950870726513e-02,   // 18
                                           3.360406198923564e-03,    // 19
                                           -3.192122997075925e-04,   // 20
                                           1.776079994155701e-05,    // 21
                                           -4.405855619446289e-07};  // 22

        m_k_AA = 1.465838128492430e+00;  // A^(-1))
        m_d_AA = 1.545206707325200e+00;  // A^(-1))
...
double x2b_A1_A1_v1x::eval(const double* xyz1, const double* xyz2, double* grad1, double* grad2, const size_t ndim,
    ...
    for (size_t j = 0; j < ndim; j++) {
        ...
        if (virial != 0) {
            (*virial)[0] += -xcrd[0] * grad1[0 + j * 3] - xcrd[3] * grad2[0 + j * 3];

            (*virial)[1] += -xcrd[0] * grad1[1 + j * 3] - xcrd[3] * grad2[1 + j * 3];

            (*virial)[2] += -xcrd[0] * grad1[2 + j * 3] - xcrd[3] * grad2[2 + j * 3];

            (*virial)[4] += -xcrd[1] * grad1[1 + j * 3] - xcrd[4] * grad2[1 + j * 3];

            (*virial)[5] += -xcrd[1] * grad1[2 + j * 3] - xcrd[4] * grad2[2 + j * 3];

            (*virial)[8] += -xcrd[2] * grad1[2 + j * 3] - xcrd[5] * grad2[2 + j * 3];
