...
/**
 * So i don't know if anything needs to be done for virtual things
 * Also I realized we don't check if the bond to be added is between
 * reasonable sites/monomers/etc
 * Should we add that? I don't remember if I asked before
 * according to the notes on my code i think i have hm.
 */
...
TEST_CASE("Bonding") {
    ...
    //testing for expected error message when looking for nonexistent bond
    bool getBonderr = false;
    try{
        s.getBond(0,0,0,1);
    }catch(const std::runtime_error &e){
        getBonderr = true;
    }
    REQUIRE(getBonderr == true);
    //checking that there are no bonds before adding any bonds
    //bool System::areSitesBonded(size_t mon1, size_t mon2, size_t idx1, size_t idx2)
    //bool System::areMonomersBonded(size_t mon1, size_t mon2)
    std::vector<size_t> sites = s.GetSitesVector();
    for(size_t i = 0; i<sites.size(); ++i){
        for(size_t j = i; j<sites.size(); ++j){
            //check that monomer bonding behavior is correct
            REQUIRE(s.areMonomersBonded(j,i) == false);
            for(size_t ii = 0; ii < sites[i]; ++ii){
                for(size_t jj = 0; jj<sites[j]; ++jj){
                    REQUIRE(s.areSitesBonded(i,j,ii,jj) == false);
                }
            }
        }
    }

    s.addBond(0,0,0,1,singleb);
    //check that we can get the bond
    REQUIRE(s.getBond(0,0,0,1) == singleb);

    //testing for expected error message when adding duplicate bonds
    bool addBonderr = false;
    try{
        s.addBond(0,0,0,1,singleb);
    }catch(const std::runtime_error &e){
        addBonderr = true;
    }
    REQUIRE(addBonderr == true);

    //check that only the added bond is added
    for(size_t i = 0; i<sites.size(); ++i){
        for(size_t j = i; j<sites.size(); ++j){
            //check that monomer bonding behavior is correct
            if(i == 0 && j == 0)
                REQUIRE(s.areMonomersBonded(j,i) == true);
            else
                REQUIRE(s.areMonomersBonded(j,i) == false);
            for(size_t ii = 0; ii < sites[i]; ++ii){
                for(size_t jj = 0; jj<sites[j]; ++jj){
                    if(i == 0 && j == 0 && ii == 0 && jj == 1)
                        REQUIRE(s.areSitesBonded(i,j,ii,jj) == true);
                    else
                        REQUIRE(s.areSitesBonded(i,j,ii,jj) == false);
                }
            }
    ...
    //add single bonds between for every site pair
    //on that note: are we allowing bonds between the same site
    for(size_t i = 0; i<sites.size(); ++i){
        for(size_t j = i; j<sites.size(); ++j){
            for(size_t ii = 0; ii < sites[i]; ++ii){
                for(size_t jj = 0; jj<sites[j]; ++jj){
                    bool badSite = false;
                    //check that bonding to a site that doesn't exist doesn't work
                    try{
                        s.addBond(i, j, ii+10, jj, singleb);
                    }
                    catch(const std::runtime_error e){
                        badSite = true;
                    }
                    REQUIRE(badSite == true);

                    if(i == 0 && j == 0 && ii == 0 && jj == 1)
                        continue;
                    
                    //check that site self-bonding doesn't exist
                    else if(i == j && ii == jj){
                        bool selfbond = true;
                        try{
                            s.addBond(i,j,ii,jj,singleb);
                        }
                        catch(const std::runtime_error e){
                            selfbond = false;
                        }
                        REQUIRE(selfbond == false);
                    }
                    else
                        s.addBond(i,j,ii,jj,singleb);
                }
    ...
    //there should now be bonds between all sites, check
    for(size_t i = 0; i<sites.size(); ++i){
        for(size_t j = i; j<sites.size(); ++j){
            //check that monomer bonding behavior is correct
            REQUIRE(s.areMonomersBonded(j,i) == true);
            for(size_t ii = 0; ii < sites[i]; ++ii){
                for(size_t jj = 0; jj<sites[j]; ++jj){
                    if(i == j && ii == jj)
                        ...
                        REQUIRE(s.areSitesBonded(i,j,ii,jj) == true);
                        //check that bond is correct type
                        REQUIRE(s.getBond(i,j,ii,jj) == singleb);
                    }
                }
            }
        }
