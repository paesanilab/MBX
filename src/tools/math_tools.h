 ...
 *
 * Given a 9 element vector (a 3x3 matrix) with the unit cell vectors in rows (v1;v2;v3), return the inverse of that matrix.
 ...
 *
 * Given a distance, an inner cutoff and an outer cutoff, calculates the switch function with formula sw(r) = [1.0 + cos( [r-ri]/[ro - ri]*PI ) ] /2.0, and its gradient with respect to r. 
 * @param[in] r Distance
 ...
 *
 * Given a double and a threshold for which a double will be considered 0, returns true if that double is lower than the threshold (i.e., is 0)
...
/**
 * @brief Given a box defined by three vectors, returns that box defined by three magnitudes and three angles (ABC alpha beta gamma notation)
 *
 * This function calculates the ABC notation for a box given in a 9-element vector with the three vectors of the box, one after the other one: {v1x,v1y,v1z,v2x...}. For reference, A,B and C are the modulus of the three vectors, while alpha is the angle between v1 and v3, beta the angle between v2 and b3, and gamma the angle between v1 and v2. V1 is supposed to be oriented along the x-axis, v2 must be in the xy plane, and v3 must have a z component.
 ...
 *
 * This function calculates the three vectors from the ABC notation for a box given in a 6-element vector with A,B,C,alpha,beta,gamma. For reference, A,B and C are the modulus of the three vectors, while alpha is the angle between v1 and v3, beta the angle between v2 and b3, and gamma the angle between v1 and v2. V1 is supposed to be oriented along the x-axis, v2 must be in the xy plane, and v3 must have a z component.
...
void MatrixTimesVector(const std::vector<T> &A, const std::vector<T> &b, std::vector<T> &c) {
    ...
    if (A.size() % b.size() != 0 ) {
        std::string text = "ERROR: Indexes do not match. " + std::to_string(A.size()) + " is not a multiple of " + std::to_string(b.size()) ;
