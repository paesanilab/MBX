...
double distance(double *p1, double *p2, std::vector<double> box, std::vector<double> box_inverse) {
    ...
    if (box.size()) {
        ...
        double dxf = box_inverse[0] * dx + box_inverse[1] * dy + box_inverse[2] * dz;
        double dyf = box_inverse[3] * dx + box_inverse[4] * dy + box_inverse[5] * dz;
        double dzf = box_inverse[6] * dx + box_inverse[7] * dy + box_inverse[8] * dz;
        ...
        dx = box[0] * dxf + box[1] * dyf + box[2] * dzf;
        dy = box[3] * dxf + box[4] * dyf + box[5] * dzf;
        dz = box[6] * dxf + box[7] * dyf + box[8] * dzf;
    }
