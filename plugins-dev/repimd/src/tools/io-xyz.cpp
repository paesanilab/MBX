...
void save_xyz(std::ostream& os, const std::string& comment, const std::vector<std::string>& elements,
    ...
    for (size_t n = 0; n < elements.size(); ++n)
        os << std::setw(5) << std::left << elements[n] << std::setw(18) << std::right << xyz[3 * n + 0] << std::setw(18)
           << std::right << xyz[3 * n + 1] << std::setw(18) << std::right << xyz[3 * n + 2] << '\n';

    os.flags(saved_flags);
}

void load_xyz(std::istream& is, std::string& comment, std::vector<std::string>& elements, std::vector<double>& xyz) {
    comment.clear();
    elements.clear();
    xyz.clear();
    ...
    if (n != natoms) {
        std::ostringstream oss;
        oss << "wrong number of atoms (" << n << " instead of " << natoms << ") in the XYZ stream";
        throw std::runtime_error(oss.str());
    }
}
